...
Fixed::Fixed(const float rawBits)
    : rawBits(static_cast<int>(roundf(rawBits * (1 << fract_bits)))) {
  // std::cout << static_cast<int>(roundf(rawBits * (1 << fract_bits))) <<
  // std::endl; std::cout << "Float constructor called" << std::endl;
}
...
int Fixed::toInt(void) const { return this->rawBits >> fract_bits; }
